...
// In normal development/production, connect to the persistent SQLite database file.
// In tests, use an in-memory database so tests are isolated and do not affect real data.
if (process.env.NODE_ENV !== "test") {
  db = new Database("/database/hr.db");
} else {
  db = new Database(":memory:");

  // Stores the available workspaces.
  db.exec(`
  ...
  // Stores work day / HYPO day values for each month in a workspace.
  db.exec(`
  ...
  // Stores zero or more specialisms per employee.
  db.exec(`
  ...
  // Stores job metadata and job-level budget/date information.
  db.exec(`
    CREATE TABLE Job (
  ...
  // Stores employee allocations against jobs, including monthly allocation columns.
  // Composite primary key means one employee can only have one row per job.
  db.exec(`
